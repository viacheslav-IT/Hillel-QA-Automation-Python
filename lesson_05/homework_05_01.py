...
def sort_cars(list_cars):
    filtered_cars = []
    for key, value in list_cars.items():
        if (
            value[1] >= search_criteria[0]
            and value[2] >= search_criteria[1]
            and value[4] <= search_criteria[2]
        ):
            filtered_cars.append([key,car_data[key]])                   # get cars that satisfy search_criteria

    sorted_cars = sorted(filtered_cars, key=lambda car: car[1][-1])     # cars should be sorted by price ascending

    for i in range(min(5, len(sorted_cars))):
        print(f"{i+1}).{sorted_cars[i][0]} {sorted_cars[i][1]}")        # print up to five (5) first found elements
